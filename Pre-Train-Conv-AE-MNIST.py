...
# -*- coding: utf-8 -*-
"""
Created on Tue Oct  9 17:34:52 2018
...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct  8 19:27:33 2018
...
class ConvAE(object):
    ...
    def partial_fit(self, X): 
        cost, summary, _,latent = self.sess.run((self.cost, self.merged_summary_op, self.optimizer,self.z), feed_dict = {self.x: X})
...
def test_face(Img, CAE, n_input):
    ...
    for i in range(5):
        ...
        plt.imshow(batch_x_test[i,:,:,0], vmin=0, vmax=1, cmap="gray") #
        plt.title("Test input")
        plt.colorbar()
        plt.subplot(5, 2, 2*i + 2)
        plt.imshow(x_re[i,:,:,0], vmin=0, vmax=1, cmap="gray")
        plt.title("Reconstruction")
        plt.colorbar()
        plt.tight_layout()
    plt.show()
    return
def get_latent_vectors(Img,CAE, batch_size):
    ...
    for i in range(n_batches):
        ...
        cost,latent = CAE.partial_fit(face_10_subjs)
        latent_.append(latent)
    latent_=np.vstack(np.asarray(latent_))
    return latent_
...
if __name__ == '__main__':
    ...
    CAE = ConvAE(n_input = n_input, n_hidden = n_hidden, learning_rate = lr, kernel_size = kernel_size, 
                 batch_size = batch_size, model_path = model_path,restore_path= model_path,logs_path= 'logs')
    lv=get_latent_vectors(Img,CAE, batch_size)
    print(lv.shape)
    np.savez('latent_vectors_mnist.npz',lv_mnist=lv)
