...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct  8 20:46:11 2018
...
if __name__ == '__main__':
    ...
    usps_dataset= sio.loadmat('Data/USPS.mat')
    usps_256=minmax_scale(usps_dataset['X'])
    
    total_usps_labels=usps_dataset['Y']-1
    Img = usps_256.reshape(-1,16,16,1)
    Label= np.ravel(total_usps_labels)
   ...
   # while iter_loop < len(all_subjects):
    ...
    batch_size = Img.shape[0]    #num_class * 64
    reg1 = 1.0
    reg2 = 0.01#1.0 * 10 ** (num_class / 10.0 - 3.0)           
    model_path = 'models_face/myusps.ckpt' 
    restore_path = 'models_face/myusps.ckpt' 
    logs_path = 'logs' 
    ...
    avg_i, med_i,pred_,true_ = test_face(Img, Label, CAE, num_class)
    avg.append(avg_i)
    med.append(med_i)
    np.savez('clustered_labels_usps.npz', pred_usps= pred_.T,true_usps=true_.T)
